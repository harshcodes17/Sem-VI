...
export const loginUser = async (req, res) => {
  try {
    ...
    const isMatch = password === user.password; // Plain-text check (for now)
...
export const changePassword = async (req, res) => {
  try {
    ...
    const hashedPassword = bcrypt.hashSync(password, 10); // Hashing password before saving

    const result = await User.changePassword(userId, hashedPassword);
    if (!result.success) {
      return res.status(400).json(new ApiError(400, result.message));
    }
